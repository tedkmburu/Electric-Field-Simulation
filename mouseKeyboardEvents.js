...
{  
    ...
    charges.forEach(charge => {
        ...
        diatanceToMouse.push(charge.pos.copy().dist(mousePosition))
    })

    closestChargeDistance = Math.min(...diatanceToMouse)
    closestChargeIndex = diatanceToMouse.indexOf(closestChargeDistance)

    if (closestChargeDistance < chargeRadius)
    {
        charges[closestChargeIndex].selected = true
    }
...
{
    let netForce = netForceAtPoint(mousePosition)
    netForce.setMag(30)
    netForce.add(mousePosition)
    foreGroundCanvas.stroke(255)
    foreGroundCanvas.line(mousePosition.x, mousePosition.y, netForce.x, netForce.y)
    // let forceVector = netForceAtPoint(mousePosition).setMag(20).add(mousePosition);
    // foreGroundCanvas.stroke(255)
    // foreGroundCanvas.line(mousePosition.x, mousePosition.y, forceVector.x, forceVector.y)
//   buttons.forEach(button => { // this will loop through all the buttons
//     if (button.visible) 
//     {
...
{
    if (charges.some(charge => charge.dragging))
    {
        charges.forEach(charge => {
            if (charge.dragging)
            {
    ...
    else
    {
        ...
        if (closestChargeDistance < chargeRadius)
        {
            charges[closestChargeIndex].pos = mousePosition.copy()
            charges[closestChargeIndex].dragging = true
        }
        else
        {
            charges[closestChargeIndex].dragging = false
